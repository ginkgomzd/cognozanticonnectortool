export default {
  // "some.translation.key": "Text for some.translation.key",
  //
  // "a": {
  //   "nested": {
  //     "key": "Text for a.nested.key"
  //   }
  // },
  //
  // "key.with.interpolation": "Text with {{anInterpolation}}"

  "location": {
    "title": "Find Local Help",
    "copy": "Need help with your health insurance application? Enter your ZIP code below to find appointments with local application assisters. You may be eligible for free or discounted insurance, so book an appointment before the February 15th deadline!",
  },

  "available": {
    "title": "Selected Location",
    "copy": "",
  },

  "confirm": {
    "thank-you": "Thank you for signing up for an appointment!",
    "on-your-way": "You’re on your way to getting covered! Here’s your appointment info",
    "what-to-bring-link": "Click here for a list of information to bring to your appointment.",
    "change-cancel": "If you need to change or cancel your appointment, please call",
    "add-to-calendar": "Add to calendar",
  },

  "form-controls": {
    "zipcode": "Enter Your ZIP Code",
    "search-radius": "Search Within",
    "language": {
      "label": "Language",
      "options": {
        "tel": "Phone translation (all languages)",
        "all": "All"
      }
    },
    "application-type": {
      "label": "Type of Application",
      "options": {
        "individual": "Individual or Family",
        "group": "Group (separate applications)",
        "shop": "Small Business (SHOP)",
      }
    },
    "start-date": "Date From",
    "end-date": "Date To",
    "first-name": {
      "label": "First Name",
      "placeholder": "First",
    },
    "last-name": {
      "label": "Last Name",
      "placeholder": "Last",
    },
    "email": {
      "label": "Email Address",
      "placeholder": "user@domain.com",
    },
    "phone": {
      "label": "Phone Number",
      "placeholder": "202-456-1111",
    },
    "opt-in-email": "Receive email updates from Get Covered America",
    "buttons": {
      "search-for-help": "Search For Help",
      "search-appointments": "Search Appointments",
      "schedule-online": "Schedule Online",
      "schedule": "Schedule",
      "find-location": "Find a Location",
      "back": "Back",
      "select": "Select",
      "default": "Search"
    },
    "results": {
      "next-available": "Next available appointment",
      "no-available": "No available appointments",
    },
  },

  "errors": {
    "no-location": "We failed to find the location you selected. Please return to the location search page and try again.",
    "no-appointment": "I'm sorry, we had some trouble loading that appointment time. Please try again.",
    "no-confirmation": "I'm sorry, something has gone wrong. Please try again.",
    "continued": "If you continue to receive this error, please contact a system administrator",
    "valid-phone": "Please specify a valid phone number",
    "no-results": "No Results Found",
    "default": "An unknown error occurred"
  },

  "app": {
    "title": "Schedule an Appointment",
    "legal": "Enrollment services are provided without discrimination on the basis of race, color, national origin, sex, age, sexual orientation, gender identity, or disability.",
    "about1": "The Health Insurance Marketplace is the way to find health care coverage that fits your budget and meets your needs.",
    "about2": "Whether you're uninsured, or just want to explore your healthcare choices, the Marketplace will help find coverage that's right for you.",
    "cognosante-tag": "Cognosante - Minds on Health",
  },

  "widgets": {
    "maplink": {
      "text": "View map",
      "hover": "Look up address on Google Maps",
    },
    "prev-next": {
      "prev": "Previous Page",
      "next": "Next Page",
    }
  },

  "words": {
    "accommodations": "Accommodations",
    "time": "Time",
    "location": "Location",
    "select": "Select",
  },

  "misc": {

  }

};
